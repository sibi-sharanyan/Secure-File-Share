...
export default class LoadingRec extends Component {
  getRandomQuote = () => {
  ...
  constructor() {
    ...
    window.ipcRenderer.on("progress-update", (event, percent , fileName) => {
  ...
  showFile = () => {
    window.ipcRenderer.send("openFile", this.state.fileName);
  }
    ...
    if (this.state.percent >= 100) {
      return (
        <div className="container-contact ">
          ...
          />
          <button className="ui positive button mt-4 mb-4" onClick = {() => this.showFile()}> Open File Location </button>
